...
コンポーネントの読み込みを必要になるまで遅らせる仕組み。

```html
@defer (on viewport) {
  <comments />          <!-- 実際に表示するコンポーネント（遅延読み込み） -->
} @placeholder {
  <p>今後のコメント</p>  <!-- 読み込み前に表示 -->
} @loading (minimum 2s) {
  <p>ロード中...</p>    <!-- 読み込み中に表示（最低2秒） -->
}
```

**流れ：**
1. ページ読み込み → `@placeholder` が表示される
2. スクロールしてビューポートに入る → `@loading` が表示され JS を取得
3. 取得完了 → コンポーネントが描画される

### なぜ速くなるか

ビルド時に `@defer` の中身が**別チャンクに分割**される。

```
dist/
  main.js            ← 初回ダウンロード
  chunk-comments.js  ← スクロールしてから初めてダウンロード
```

初回の `main.js` が小さくなる分、ページの表示開始が速くなる。

### トリガーの種類

| トリガー | タイミング |
|---|---|
| `on viewport` | 要素が画面内に入ったとき |
| `on idle` | ブラウザがアイドル状態のとき |
| `on interaction` | クリック・フォーカスされたとき |
| `on hover` | ホバーされたとき |
| `on timer(3s)` | 3秒後 |
| `when 条件式` | 式が true になったとき |

**注意：** 最初から画面内に見えるコンポーネントを `@defer` にすると、追加リクエストの分だけ表示が遅れる。スクロールしないと見えない要素に使うのが基本。
...
```
DNS解決       ドメイン → IP アドレスに変換      数ms〜数十ms
接続コスト    TCP + TLS のハンドシェイク          50〜200ms
転送コスト    データを送受信する時間              ファイルサイズ次第
```

何がボトルネックかはケースによる：
- 小さいファイルが多い → 接続コストが支配的
- 大きいファイル1個 → 転送コストが支配的

### オーバーヘッドとは

本来の目的以外にかかる余分なコストのこと。HTTP リクエストでは毎回送られるヘッダー情報がオーバーヘッドにあたる。

```
GET /chunk-comments.js HTTP/1.1
Host: localhost:4200
User-Agent: Mozilla/5.0 ...   ← データ本体と関係ない付随情報
Cookie: ...                      = オーバーヘッド
```

### HTTP/2 の多重化

HTTP/1.1 は1接続で1リクエストずつ。接続コストがリクエストごとにかかる。

HTTP/2 は**1本の接続の中に複数のストリームを作り並列処理**する。接続コストが1回で済むのでリクエスト数が増えても痛くない。ヘッダーも圧縮されオーバーヘッドが小さい。

→ `@defer` でチャンクが増えても、HTTP/2 環境では接続コストは変わらず初回ダウンロード量が減る恩恵の方が大きい。
